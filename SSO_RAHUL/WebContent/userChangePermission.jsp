...
<html>
<title>USER CHANGE RESOURCES</title>
					...
					<tr>
						<td width="463" background="images/index_17.gif"><table
								...
								<tr>
									...
									%>
									<td><a
										href="userSetPermission?read=n&&rid=<%=rs1.getResId()%>&&uid=<%=request.getParameter("uid")%>">DENY</a>
									</td>
									<%
										} else {
									%>
									<td><a
										href="userSetPermission?read=y&&rid=<%=rs1.getResId()%>&&uid=<%=request.getParameter("uid")%>">ALLOW</a>
									</td>
									<%
										}
									%>
									...
									%>
									<td><a
										href="userSetPermission?write=n&&rid=<%=rs1.getResId()%>&&uid=<%=request.getParameter("uid")%>">DENY</a>
									</td>
									<%
										} else {
									%>
									<td><a
										href="userSetPermission?write=y&&rid=<%=rs1.getResId()%>&&uid=<%=request.getParameter("uid")%>">ALLOW</a>
									</td>
									<%
										}
									%>
									...
									%>
									<td><a
										href="userSetPermission?exec=n&&rid=<%=rs1.getResId()%>&&uid=<%=request.getParameter("uid")%>">DENY</a>
									</td>
									<%
										} else {
									%>
									<td><a
										href="userSetPermission?exec=y&&rid=<%=rs1.getResId()%>&&uid=<%=request.getParameter("uid")%>">ALLOW</a>
									</td>
									<%
										}
									%>
